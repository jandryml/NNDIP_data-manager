...
import static cz.edu.upce.fei.datamanager.views.actionconfig.ActionConfigFormLegacy.*;

@PageTitle("Action config - Legacy")
@Route(value = "actions-legacy", layout = MainLayout.class)
@PermitAll
public class ActionConfigViewLegacy extends VerticalLayout {
    Grid<ThresholdAction> grid = new Grid<>(ThresholdAction.class);
    TextField filterText = new TextField();
    ActionConfigFormLegacy form;
    private ThresholdActionService thresholdActionService;

    public ActionConfigViewLegacy(ThresholdActionService thresholdActionService) {
    ...
    private void configureForm() {
        form = new ActionConfigFormLegacy();
        form.setWidth("25em");
    ...
    private void configureGrid() {
        ...
        grid.addColumn(contact -> contact.getOutputType().name()).setHeader("Register type");
